    ...
    for (const match of matches) {
      ...
      try {
        ...
        try {
            ...
            if (!currentSupabasePlayerId) {
            console.error(`Skipping POTM for player ${potmPlayerId} as Supabase player ID could not be determined.`);
            await logger.logPlayerProcessing(
                'get POTM player', 
                `Player ${potmPlayerId}`, 
                potmPlayerId, 
            ...
            let matchData;
            if (existingData) {
                // Update existing record with POTM bonus
                matchData = {
                ...existingData,
                bonus_potm: true,
                match_status: matchStatus,
                ...
                // Create new record with only POTM bonus (player didn't bat/bowl/field)
                matchData = {
                match_id: match.id,
                tournament_id: match.tournament_id,
                match_type: match.type_match,
...
async function saveMatchJson(supabase, match, apiData) {
  try {
    ...
    if (error) {
      ...
      if (error.code === '23505') {
        console.log(`JSON archive already exists for match ${match.id}, skipping insert.`);
      } else {
