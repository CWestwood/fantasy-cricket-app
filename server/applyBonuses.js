...
async function applyBonuses() {
  ...
  if (!bonuses.length) {
    console.log("ℹ️ No pending bonuses found.");
    return;
  }
  ...
  for (const bonus of bonuses) {
    try {
      console.log(`➡️ Processing bonus for match ${bonus.match_id}, player ${bonus.player_id}`);

      // 2️⃣ Update match_data 
      ...
      // 3️⃣ Recalculate points for this player in this match
      ...
      if (bonusPoints > 0) {
  // 1️⃣ Fetch current scores for this player in the match
